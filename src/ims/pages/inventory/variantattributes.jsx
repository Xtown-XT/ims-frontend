...
const VariantAttributes = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const res = await variantAttributesService.getVariantAttributes(
          page,
          limit,
          search
        );
        ...
        setTotal(
          res.data?.data?.count || res.data?.count || attributes.length
        );
      } catch (err) {
  ...
  const handleAddVariant = async () => {
    ...
    try {
      ...
      message.error(
        err.response?.data?.message || "Failed to add variant attribute"
      );
    }
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      await variantAttributesService.updateVariantAttribute(
        editRecord.id,
        payload
      );
  ...
  const confirmDelete = async () => {
    try {
      await variantAttributesService.deleteVariantAttribute(
        deleteRecord.id
      );
  ...
  const handleExportCSV = () => {
    ...
    const dataToExport =
      filteredData.length ? filteredData : formData;
    const headers = [
      "variant",
      "values",
      "createddate",
    ...
    link.setAttribute(
      "download",
      `variantattributes_export_${new Date()
        .toISOString()
        .slice(0, 10)}.csv`
    );
  ...
  const handleExportPDF = () => {
    ...
    doc.save(
      `variantattributes_${new Date().toISOString().slice(0, 10)}.pdf`
    );
        ...
        <Form layout="vertical">
          ...
          <Form.Item label="Status">
            <Switch
              ...
              style={{
                backgroundColor: addVariantData.status
                  ? "#3EB780"
                  : "#ccc",
              }}
            />
